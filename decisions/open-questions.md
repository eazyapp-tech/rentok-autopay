...
| 24 | e-NACH stays a small "Use bank account instead" link, and is not made the default rail above ₹15,000 | R44 makes it a link. Kamal's launch room calls it the right rail above ₹15,000: one presentment instead of three parts, about twenty times cheaper on PayU's rates, no exposure to the ₹1,00,000 daily ceiling, and a mandate she cannot switch off from her own app. Decides how every rent above ₹15,000 is collected |
...
| 28 | The Platform fee is a ladder of fixed published prices by the property's average rent, and the manager app suggests the rung. Sanchay's instruction of 22 Sep. Two numbers are his: whether the floor moves below R53's ₹58, which is 57.6% of tenants, and where the ceiling goes, since R53's ₹118 cannot serve the 5.7% above it. Recommended: hold the floor, raise the ceiling to the top rung | R53's band cannot express a property whose tenants pay ₹1,00,000. Proposal, portfolio bands and the reason a ladder is allowed where a rate is not: `research/platform-fee-ladder.md` |
| 29 | The "Eligible for tenants joined since" control on manager web is removed, or ruled in and specified | It is on screen, it saves a date to the property and writes an activity log line, and **no Autopay path reads it**. So an owner who sets it is told his older tenants are excluded and they are not. It looks like an unfinished feature, so the likely future is that someone wires it up, which would give every property with a date saved a silent wave against R37 |

Items 10 to 18 were added on 22 Sep 2026 from the manager build tickets D1, D2 and D3, where they were recorded as proposals and would not have been seen here. Items 19 to 22 were added the same day from Kamal's launch room PRD and his payment page prototype, where each one is designed and built but disagrees with a ruling. Item 23 was added the same day from the tenant app, where a ticket names an app that does not exist. Item 29 came from the manager web surface, where a live control is read by nothing. Item 28 came from Sanchay's instruction of 22 Sep that the fee is suggested from average rent, and corrects a clause of R63 that Claude had written too broadly. Item 27 came from re-checking Kamal's three earlier documents (`research/kamal-earlier-material-check.md`), which measured the figure the whole project is sized on. Items 24 to 26 came from checking all 23 sections of Kamal's launch room (`research/kamal-launch-room-check.md`), which also answers 6 and 23 in practice without a ruling.

...
| ~~22~~ | **Answered 23 Sep as R71:** Rent Coins and the Hubble store are in the launch; coins, not cash; 100 coins = ₹1 as a backend setting | Points on every rupee of rent, a grant for setting up Autopay and a voucher catalogue are in the PRD and the prototype, and in no ruling. RentPass cashback is parked until a partner funds it, because RentOk absorbs nothing (R11) |
...
| 30 | The tenant pauses her Autopay from the payment page herself, with no approval from the property | **A fifth conflict between Kamal's prototype and a ruling, found 23 Sep and not caught in the first check.** R48 makes a pause a **request** the property approves, with silence counting as yes after 48 hours (R50). His prototype's mandate screen gives the tenant a plain "Pause one month" button and a "Start again from {month}" line, with no request, no approval and no waiting. One of the two has to give. Two smaller facts sit under it: the same screen tells her "you can also pause or stop this from your own UPI app", and our own reading of Cashfree's docs (I10) says pause is **not supported on on-demand mandates**, which is the option most tenants are meant to pick. So the screen may be promising something the rail does not do. **23 Sep workflow call:** the room treated pause as hers, with no approval mentioned, RentOk-side only (we stop debiting; the mandate stays live at Cashfree), to a date she picks, from the next cycle only (90:38). Claude's refinement to put to Sanchay: allow a pause for this cycle until the bank's notice goes out, and from the next cycle after that |
| 31 | **The ceiling at launch.** In the 23 Sep workflow call, "All my monthly dues" approves ₹15,000 for everyone "for now" (73:13). R68, logged earlier that afternoon, sets fixed dues plus ₹2,000, rounded up to the next ₹5,000, never above ₹15,000 | Decides whether a ₹6,000 tenant approves ₹10,000 or ₹15,000. Both work without a page change, because the page reads the ceiling from the backend. Claude's pick: R68 from day one |
| 32 | **Charges at launch.** The same call ships the first release on today's live charge settings and leaves "who bears what" for later (16:38) | R64 deletes the Autopay setup fee and makes one Platform fee the tenant's charge. Shipping first on live settings means launch runs on the model R64 replaced |
